...
Chương này trình bày chi tiết các công nghệ, nền tảng và cơ sở lý thuyết được lựa chọn để hiện thực hóa hệ thống chia sẻ thực phẩm đã được đặc tả ở Chương 2.

\section{Tổng quan kiến trúc và ánh xạ yêu cầu}
Hệ thống bao gồm ba thành phần chính: ứng dụng di động dành cho ba vai trò người dùng (Người cho (Donor), Người nhận (Receiver), Tình nguyện viên (Volunteer)), website dành cho quản trị viên (admin) và máy chủ phụ trợ (backend) cung cấp REST API kết hợp kênh truyền thông thời gian thực. Bảng~\ref{tab:anh_xa_yeu_cau} dưới đây thể hiện sự ánh xạ từ các yêu cầu cốt lõi ở Chương 2 sang nhóm công nghệ được sử dụng để giải quyết.
...
Lý thuyết này giải quyết triệt để yêu cầu trò chuyện và cập nhật trạng thái đơn hàng theo thời gian thực. Phương án giả lập realtime trên HTTP đã bị loại bỏ do có độ trễ cao và gây lãng phí tài nguyên máy chủ.

\subsection{Định vị địa lý và tính khoảng cách}
Vị trí người dùng được biểu diễn bằng cặp tọa độ \textbf{vĩ độ/kinh độ} theo hệ quy chiếu \textbf{WGS-84}, chuẩn tọa độ phổ biến của GPS và các dịch vụ bản đồ. Để giải quyết bài toán lọc bài đăng/đơn hàng theo bán kính và ước lượng quãng đường, hệ thống áp dụng hai phương pháp:
(i) \textbf{Khoảng cách đường chim bay (Công thức Haversine)} \cite{MovableTypeLatLong}: Dùng để tính khoảng cách giữa hai điểm trên mặt cầu Trái Đất (với bán kính $\approx$ 6371 km). Công thức được áp dụng như sau:
    \begin{equation}
    \begin{aligned}
...
trong đó $\varphi$ là vĩ độ, $\lambda$ là kinh độ (đã chuyển sang radian). Đây là phép toán nội bộ, tốc độ cao, không phụ thuộc mạng, dùng để sắp xếp danh sách đơn theo khoảng cách, và lọc đối tượng nhận thông báo đẩy: 10 km cho donor, 20 km cho tình nguyện viên.".

(ii) \textbf{Khoảng cách đường bộ thực tế (Google Distance Matrix API)} \cite{GoogleDistanceMatrixAPI}: Hệ thống gọi API để lấy quãng đường di chuyển thực tế. Dữ liệu được kết hợp bộ nhớ đệm (cache) với thời gian sống (TTL) 10 phút và làm tròn tọa độ $\sim$111 m để tối ưu chi phí hạn ngạch. Nếu gặp sự cố mạng, hệ thống tự động dự phòng bằng công thức Haversine.
...
(i) \textbf{JSON Web Token}: Một token bao gồm ba phần (header, payload, signature). Phần chữ ký được tạo bằng thuật toán HMAC, kết hợp hàm băm với khóa bí mật nhằm bảo toàn tính toàn vẹn [22]. Máy chủ chỉ cần xác minh chữ ký bằng \texttt{JWT\_SECRET} là có thể cấp quyền, hoàn toàn phù hợp với mô hình không trạng thái.
(ii) \textbf{Hàm băm mật khẩu một chiều}: Mật khẩu luôn được băm trước khi lưu trữ. Thuật toán bcrypt bổ sung chuỗi ngẫu nhiên (chống tấn công rainbow table) và hệ số công (work factor) nhằm làm chậm quá trình băm, khiến việc dò tìm mật khẩu trở nên bất khả thi trong thời gian ngắn.
...
Express là framework tối giản nhưng sở hữu hệ thống định tuyến và middleware vô cùng linh hoạt. Kiến trúc được phân tầng chuẩn mực (\textit{routes $\rightarrow$ controllers $\rightarrow$ services $\rightarrow$ models}). Việc dùng chung ngôn ngữ lập trình giữa client và server mang lại lợi thế lớn. Thay vì sử dụng NestJS (quá cồng kềnh) hay Spring Boot/Django (khác ngôn ngữ), Node.js kết hợp cùng thư viện \textbf{Axios} ở client mang lại sự đồng bộ và hiệu năng hoàn toàn đáp ứng được quy mô đồ án.
...
Mô hình NoSQL hướng tài liệu (document-oriented) của MongoDB khớp tự nhiên với dữ liệu JSON trao đổi qua REST API [6]. Mongoose bổ sung lược đồ, giúp kiểm tra ràng buộc tham chiếu chặt chẽ ở tầng ứng dụng [7]. CSDL quan hệ (như PostgreSQL) không được ưu tiên do nghiệp vụ hệ thống ít phát sinh các giao dịch ACID phức tạp, đồng thời MongoDB mang lại sự linh hoạt cao khi cần mở rộng thông tin hồ sơ.

\subsection{Giao tiếp thời gian thực (Realtime)}
\textbf{Lựa chọn:} Socket.IO.\\
Cung cấp kênh truyền hai chiều mạnh mẽ dựa trên WebSocket, có sẵn cơ chế tự động kết nối lại và phân phòng [8]. Giải pháp này nhẹ, tích hợp gọn gàng vào Express server, không gây độ trễ như HTTP polling định kỳ và không tốn chi phí phụ thuộc vào bên thứ ba (như Pusher).

\subsection{Xác thực và phân quyền}
Hệ thống sử dụng \textbf{JWT} lưu trữ an toàn trong \texttt{expo-secure-store} cho phiên làm việc; \textbf{bcrypt} để bảo mật mật khẩu.
...
\section{Kết luận}
Backend được đóng gói vào các ảnh container bằng \textbf{Docker} nhằm đảm bảo môi trường vận hành nhất quán tuyệt đối giữa lúc phát triển và lúc đưa lên máy chủ thực tế. Quyết định lựa chọn công nghệ bám sát ba tiêu chí: (i) Thống nhất ngôn ngữ JS/TS xuyên suốt hệ thống; (ii) Lựa chọn giải pháp "đủ dùng", tài liệu dồi dào, phù hợp với thời lượng đồ án; (iii) Giải quyết trực tiếp các bài toán ở Chương 2. Các phương án thay thế đều đã được cân nhắc kỹ lưỡng và hệ thống được thiết kế mở, hoàn toàn sẵn sàng tích hợp các dịch vụ đám mây tiên tiến khi cần mở rộng quy mô.
