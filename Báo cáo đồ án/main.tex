...
\usepackage{longtable} % Cho phép bảng dài tự ngắt qua nhiều trang
\usepackage{booktabs} % Tạo bảng với đường kẻ đẹp
...
\newcommand{\chaptergraphic}[2][]{%
  \IfFileExists{../#2}{%
    ...
    \IfFileExists{#2}{%
      ...
      \fbox{%
        \begin{minipage}[c][4cm][c]{0.85\linewidth}
          \centering\small
          Hình chưa được tìm thấy:\\[0.5em]
          \texttt{\detokenize{#2}}
        \end{minipage}%
      }%
      \endgroup
...
\lstset{basicstyle=\ttfamily\small, breaklines=true, columns=fullflexible} % Định dạng code
...
% Tạo danh mục công thức toán học.
\makeatletter
\newcommand{\listequationname}{DANH MỤC CÔNG THỨC}
...
\providecommand*{\l@equation}{\@dottedtocline{1}{0em}{6.5em}}
\makeatother
...
\newpage
% Tạo danh mục công thức.
{\let\oldnumberline\numberline%
\renewcommand{\numberline}{Công thức~\oldnumberline}
\listofequations}
